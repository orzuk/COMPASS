...
%
function [ReadBySpeciesMat kmers_packed] = BuildMixingMatrixFromSequencesRegions( ...
...
if(exist('kmer_seq_inds', 'var') && (~isempty(kmer_seq_inds)))
  ...
  %      extract_sub_kmersRegions(packed_seqs, seqs_lens, readLength, unique_flag, 0, ...
  %      kmer_seq_inds, kmer_positions_inds); % Call c-function (no hash!!!) with pre-specified locations

  [kmers_packed kmers_inds] = ...
        extract_sub_kmers6RegionJan13(packed_seqs, seqs_lens, readLength, unique_flag, 0, ...
        kmer_seq_inds, kmer_positions_inds); % Call c-function (no hash!!!) with pre-specified locations


else                     
    [kmers_packed kmers_inds] = ...
        extract_sub_kmers6RegionJan13(packed_seqs, seqs_lens, readLength, unique_flag, 0); % Call c-function (no hash!!!)
end
