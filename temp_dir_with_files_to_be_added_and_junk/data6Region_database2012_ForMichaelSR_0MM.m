...
P = load([bcs_data_path,'6region/primers_position_few_regions_0MM_database780000']);
...
save([bcs_data_path,'full16S/data_primersAmit_0MM_databaseOf2012_readLength_76_noACGT_onlySS'],'SS') 
...
save([bcs_data_path,'full16S/data_primersAmit_0MM_databaseOf2012_readLength_76_noACGT'],'sameSeq','seq_readLength76','M','values') 

%whos x sameSeq   seq_readLength76   M   values 

% do forward and reverse separately

clear
bcs_data_path = '~/CS/BAC/';
load([bcs_data_path,'full16S/data_primersAmit_0MM_databaseOf2012_readLength_76_noACGT_onlySS'])

numBact = size(SS.Sequence_packed_matrix,1);
readLength = 76;
clear values_f values_r M_f M_r 
for reg=1:6
  reg
  % output the bacteria in each region - only forward
  [PrimerReadBySpeciesMat_tmp Primer_kmers_packed_tmp bactInRegion_tmp]=oneRegionDataForwardOnly(reg,readLength,SS);
  
  [junk,i1] = intersect(1:numBact,bactInRegion_tmp);
  M_f{reg} = sparse(size(PrimerReadBySpeciesMat_tmp,1),numBact);
  M_f{reg}(:,i1) = PrimerReadBySpeciesMat_tmp(:,bactInRegion_tmp);
  values_f{reg} = Primer_kmers_packed_tmp;

  % output the bacteria in each region - only reverse
  [PrimerReadBySpeciesMat_tmp Primer_kmers_packed_tmp bactInRegion_tmp]=oneRegionDataReverseOnly(reg,readLength,SS);
  
  [junk,i1] = intersect(1:numBact,bactInRegion_tmp);
  M_r{reg} = sparse(size(PrimerReadBySpeciesMat_tmp,1),numBact);
  M_r{reg}(:,i1) = PrimerReadBySpeciesMat_tmp(:,bactInRegion_tmp);
  values_r{reg} = Primer_kmers_packed_tmp;
end

save([bcs_data_path,'full16S/data_primersAmit_0MM_databaseOf2012_readLength_76_noACGT_M_values_forward_reverse_separated'],'M_f','values_f','M_r','values_r'); 
