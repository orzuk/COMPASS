...
basicOffSet = 6*readLength;
new_ord = [];
for i=1:6
 new_ord = [new_ord,(i-1)*readLength+1:i*readLength,basicOffSet+(i-1)*readLength+1:basicOffSet+i*readLength];
end

seq_readLength76_PE = seq_readLength76(:,new_ord);
...
for i=1:numBact
  ...
  curr_seq = seq_readLength76_PE(i,:);
...
save([bcs_data_path,'full16S/data_primersAmit_0MM_databaseOf2012_readLength_76_noACGT_onlySS_forPE'],'SS') 
...
for reg=1:6
  ...
  [PrimerReadBySpeciesMat_tmp Primer_kmers_packed_tmp bactInRegion_tmp]=oneRegionDataForPE(reg,readLength*2,SS);
...
save([bcs_data_path,'full16S/data_primersAmit_0MM_databaseOf2012_readLength_76_noACGT_forPE'],'sameSeq','seq_readLength76_PE','M','values') 
