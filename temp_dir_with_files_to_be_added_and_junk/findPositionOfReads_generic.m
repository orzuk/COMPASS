function findPositionOfReads_generic(auxData)
...
load([auxData.inputDirName,'/',auxData.readsFileName])


n = size(uniqueReads,1);
...
runFileName = [userDir,'/CS/BAC/findAlignment_generic/tmpRuns/',auxData.tmpBasicFileName,'/',auxData.tmpBasicFileName];
basicAllocationFileName = [userDir,'/CS/BAC/findAlignment_generic/dataForSim/',auxData.tmpBasicFileName,'/',auxData.tmpBasicFileName];
currDir = [userDir,'/CS/BAC/findAlignment_generic/tmpRuns/',auxData.tmpBasicFileName];
basicSaveName = [userDir,'/CS/BAC/findAlignment_generic/',auxData.tmpBasicFileName];
...
if strcmp(parallelType,'cluster')
  ...
  for j=1:length(tmpInd)
    ...
    % run one part - generic format test
    runOneNodeBasedFindPositionOfReads_generic(mFileName,saveFileName,tmpInd{j}(1),tmpInd{j}(end),auxData)
  end
  ...
  if auxData.searchForwardAndReverseFlag==1
    POS = zeros(1,n);
    SD = POS;
    REV_POS = POS;
    REV_SD = POS;
    ...
    save([basicSaveName,'_finalRes'],'POS','SD','REV_POS','REV_SD')	
  else % only forward
    POS = zeros(1,n);
    SD = POS;
    for j=1:length(tmpInd)
    ...
    save([basicSaveName,'_finalRes'],'POS','SD')	
  end
  
  
  
	
  unix(['rm -f ',currDir,'/core*; ','rm -f ',runFileName,'_*; ']);
