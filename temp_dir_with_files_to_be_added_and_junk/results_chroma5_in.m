...
function [errorsMinorOverSequence,reordered_cData]=results_chroma5_in(basicDirName,fileName,cData,SangerSeq_start,AmplifiedSeq_start,lengthValidSanger,pos_xl,tmp_ind_W_454,tmp_ind_W_Solexa,sameNumbers,paperPosition,thresh,fs,th_sec2first)
%keyboar
lenName = 50;
...
for i=1:length(reordered_cData.Numbers)
  ...
  lab{i} = ['{\color{magenta}[',num2str(currNumber),' ',num2str(overlapData.amp(i,:)),']};'];
  [junk,ind] = intersect(found.s750(:,1),currNumber);
  if ~isempty(ind)
    lab{i} = [lab{i},'\t {\colorbox{blue}{\color{blue}}{[',num2str(found.s750(ind,2)),' ',num2str(overlapData.s750(i,:)),']}};'];
  else
    lab{i} = [lab{i},'\t {\color{gray} [',num2str(overlapData.s750(i,:)),']};'];
  end
...
%format_ticks(gca,xl,toPrint,[pos_xl],[1:20],[],[],[],'fontweight','bold','fontsize',fs)
