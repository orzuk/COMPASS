function [X,sumRelevantReads]=iterateParallelDistributedSequenceFilesOrFourth(uniqueReads,uniqueReads_length,curr_kp,basicSeqNameDir,basicSeqKey,parallelType,basicSaveName,userDir,runFileName,auxData)
%keyboard
...
if strcmp(parallelType,'local')
  ...
  %keyboard
    ...
    for i=1:length(tmpInd)
      [res.x,res.sumRelevantReads]=solveForGroupOrFourth(i,i,auxData,[],[],tmpInd,basicSeqNameDir,basicSeqKey,readLength,uniqueReads,uniqueReads_length);
      x{i} = res.x{i};
