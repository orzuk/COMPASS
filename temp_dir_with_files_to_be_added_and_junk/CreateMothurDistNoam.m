...
function [distmat,allSeqs,origFreqVec,recFreqVec]=CreateMothurDistNoam(dirName,fileName,outfilename,israndom,auxData)
...
a = findstr(fileName,'.mat');

tmpfilename = ['seqs_',fileName(1:a-1)];
...
%keyboard
w = ['!~/CS/BAC/mothur/mothur "#align.seqs(candidate=',tmpfilename,'.fa'...
     ', template=~/CS/BAC/core_set_aligned.fasta.imputed); dist.seqs(fasta=',...
...
distmat=dlmread([tmpfilename,'.square.dist'],'\t',0,1);
distmat=distmat(2:end,1:end-1);

% save the output
save([dirName,outfilename],'distmat','allSeqs','origFreqVec','recFreqVec');
