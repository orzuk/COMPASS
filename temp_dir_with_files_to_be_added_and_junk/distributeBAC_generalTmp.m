function distributeBAC_generalTmp(auxDataFileName)
% reads are red
...
load(auxData.reads_data_fileName,'ind_bac_in_mix')
auxData.ind_bac_in_mix = ind_bac_in_mix;
%keyboard
curr_kp = 251001:252000
[currX,currSumRelevantReads] = iterateParallelDistributedSequenceFiles4(uniqueReads,uniqueReads_length,curr_kp,basicSeqNameDir,basicSeqKey,parallelType,[basicSaveName,'_k_',num2str(k)],userDir,[runFileName,'_k_',num2str(k)],auxData);
store_kp{k} = curr_kp;
store_X{k} = currX;
...
if strcmp(parallelType,'local')
  ...
  solveAgain4(saveSolveAgainDataName,[auxData.saveName,'1']);
else
...
if 1==2
  ...
  Nbac_in_mixture = 2000;
  Nreads = 2e6;
  readlen = 50;
  npower = 0.5;
  bac_dist_flag = 1;
  outfilename = '~/CS/BAC/dataForSim/sim4';
  N = 2001;
  createReadsFlag = 0;
  ...
  auxData.minimalNumberOfDependent = 7;
  ...
  auxData.numProcessors = 4;
  auxData.groupSize = 800;
  auxData.smallestSetOfCollected = 1000;
  auxData.numBACtoConsider = 2001
  auxData.thresholdForCollectingBAC = 1e-3;
  auxData.firstFlag = 1;
  auxData.createReadsFlag = 1;
  auxDataFileName = ['~/CS/tmp/structFor_',auxData.tmpFileName];
  save(auxDataFileName,'auxData')
  distributeBAC_general4(auxDataFileName)
  ...
  % 500
  clear
  strinfilename = '/homes/csfaculty/shental/CS/BAC/dataForSim/simVariableNreadsClose_2000000_Nbacmix_500_Nread_2000000_Readlen_50_Npower_05_bacdistflag_1_NoReads';
  
  
  auxData = struct;
  auxData.reads_data_fileName = strinfilename;
  auxData.minimalNumberOfDependent = 7;
  auxData.tmpFileName = 'test500Close';
  auxData.currDir = ['/homes/csfaculty/shental/CS/BAC/tmpRuns/',auxData.tmpFileName];
  unix(['mkdir ',auxData.currDir]);
  unix(['cd ',auxData.currDir]); 
