function [refinedMat, values, independentInd,dependentInd]=prepareGroupOf1000DistributedSequenceFiles4(readLength,tmpInd,basicSeqNameDir,basicSeqKey,deleteDependentFlag)

if ~exist('deleteDependentFlag')
    deleteDependentFlag = 1;
end

% the same as prepareGroupOf1000DistributedSequenceFiles2 but check the rank and deletes rows with zeros

% Sequence1 are the sequences of the current group
% load Sequence1 based on tmpInd
load(basicSeqKey,'positionInPart')
Sequence1 = cell(length(tmpInd),1);
%keyboard
for i=1:length(tmpInd)
  clear seq_*
  load([basicSeqNameDir,'seq_part_',num2str(positionInPart(tmpInd(i)))],['seq_',num2str(tmpInd(i))]);
  w = ['Sequence1{i} = seq_',num2str(tmpInd(i)),'{1};'];
  eval(w);
end
clear seq_* positionInPart

% seq is the list of possible sequences for each Sequence1
o = 1:length(Sequence1);
for i=1:length(o)
  if mod(i,1000)==0
    i
  end
  seq{i} = char(55*ones(length(Sequence1{o(i)})-readLength,readLength));
  for j=0:length(Sequence1{o(i)})-readLength
    seq{i}(j+1,:) = Sequence1{o(i)}(j+1:j+readLength);
  end
  seq{i} = uint8(seq{i});
end  
%keyboard
clear Sequence1 Sequence_mix

posO = [];
seqO = [];
ln = zeros(1,length(o));
for i=1:length(o)
  ln(i) = size(seq{i},1);
end
len = sum(ln);

% SeqO is a list of all sequences in all Sequence1
k = 0;
%seqO = uint8(char(55*ones(len,readLength,'uint8')));
seqO = zeros(len,readLength,'uint8');
posO = zeros(len,1,'uint16');
for i=1:length(o)
  seqO(k+1:k+length(seq{i}),:) = seq{i};
  posO(k+1:k+length(seq{i})) = i*ones(length(seq{i}),1);
  k = k+length(seq{i});
end
clear seq

%%%%%%%
% find the reads in each of these
%keyboard
% output the data
disp('assumes that the number of bacteria in part is smaller than 2^16')
%keyboard
[values,currInds,positions,leng] = myUniqueUINT8(seqO,posO);


if 1==2
  tic;[values1,currInds1,positions1,leng1] = myUniqueUINT8new(seqO,posO);toc
  find(values1-values)
  find(leng1-leng)
  for j=1:length(currInds)
    if length(currInds{j})~=length(currInds1{j}) || ~isempty(find(currInds{j}-currInds1{j}))
      j
      pause
    end
    
  end
  for j=1:length(positions)
    if length(positions{j})~=length(positions1{j}) || ~isempty(find(positions{j}-positions1{j}))
      j
      pause
    end
  end
end

% values = the unique rows in Esq
% currents = the indices of the unique rows in seqO
% positions = the BAC which holds the sequence and the number of times it does.
% leng = the total number of BAC which hold the sequence

clear seqO currInds posO

%keyboard
[vals_leng inds_leng] = get_duplicates(leng);
% inds_leng = indices of unique values of leng - the interesting one is leng==1

lenb = sum(leng);
clear leng

% inds_leng = indices of 

%pause
keep = 1:size(values,1);


lenKeep = length(keep);


% change to uint* ? based on number of sequences: size(values)
put = zeros(lenb,3);
k = 0;
for i=1:length(inds_leng)
  
  i1 = 1:length(inds_leng{i});

  b = inds_leng{i}(i1)*ones(1,vals_leng(i));
  
  dat = double(cell2mat(positions(inds_leng{i})));
  curr_pos = dat(1:2:end,:);curr_pos = curr_pos(i1,:);
  curr_num = dat(2:2:end,:);curr_num = curr_num(i1,:);  
  
  % curr_dat: [sequence i; bacteria j;number of times]
  curr_dat = [b(:) curr_pos(:) curr_num(:)];
  
  put(k+1:k+size(curr_dat,1),:) = curr_dat;
  k = k+size(curr_dat,1);
  
end

clear positions inds_leng d
% change this - to include values themselves and the work on the recuded number of rows


part = 1:10^5:lenb;
if part(end)<lenb
  part(end+1) = lenb;
end
part = [part,lenb+1];

normalizedBac = spalloc(length(keep),length(o),lenb);
for i=1:length(part)-1
  
  inWhichBAC = spalloc(length(keep),length(o),lenb);
  do = part(i):part(i+1)-1;
  inWhichBAC((put(do,2)-1)*lenKeep+put(do,1)) = put(do,3);
  normalizedBac = normalizedBac+inWhichBAC;
end
clear put
nnz(normalizedBac)
clear inWhichBAC keep

for i=1:length(o)
  normalizedBac(:,i) = normalizedBac(:,i)/ln(i);
end
%keyboard


% check the rank
%keyboard

if deleteDependentFlag==0 % for example for calculating infinite number of reads for a specific set - we do not delete the deepenent
  disp('create the reads for infinite number of reads')
  refinedMat = normalizedBac;
  values = values;
  independentInd = [];
  dependentInd = [];
  return
  
end

fm = full(normalizedBac'*normalizedBac);
rnk = rank(fm);


if rnk<size(normalizedBac,2)
  D = rref_mod2(fm);
  %then the indices you need to keep (which are independent) are:
  independentInd = sum(D~=0)==1;
  
  dependentInd = 1:size(normalizedBac,2);
  dependentInd(independentInd) = [];
  
  refinedMat = normalizedBac;
  refinedMat(:,dependentInd) = [];
  
 
  delRowsFroNormalizedBAC = find(sum(refinedMat,2)==0);
  if ~isempty(delRowsFroNormalizedBAC)
    disp('deleting rows from normalizedBac. prepareGroupOf1000DistributedSequenceFiles4')
    pause
    %keyboard
  end
  refinedMat(delRowsFroNormalizedBAC,:) = [];
  values(delRowsFroNormalizedBAC,:) = [];
  
  independentInd = tmpInd(independentInd);
  dependentInd = tmpInd(dependentInd);
else
  disp(['num of bac: ',num2str(length(tmpInd)),'. all are independent'])
  refinedMat = normalizedBac;
  independentInd = tmpInd;
  dependentInd = [];
end
