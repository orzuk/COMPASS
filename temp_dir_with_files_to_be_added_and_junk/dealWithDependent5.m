function [x_extra,ind_extra]=dealWithDependent5(dependentInd,minimalNumberOfDependent,basicAllocationFileName,tmpInd,basicSeqNameDir,basicSeqKey,readLength,uniqueReads,uniqueReads_length)
...
formerLength = inf;
%keyboard
while length(currDep{iterDepCounter})>minimalNumberOfDependent & length(currDep{iterDepCounter})<formerLength
  
  formerLength = length(currDep{iterDepCounter});
  clear res_extra
  [res_extra.x,res_extra.sumRelevantReads,res_extra.independentInd,res_extra.dependentInd]=solveForGroup5(1,1,[],currDep(iterDepCounter),basicSeqNameDir,basicSeqKey,readLength,uniqueReads,uniqueReads_length);
