...
%
% X=MADLIN(C,d,A,b,Aeq,beq,LB,UB,X0) sets the starting point to X0.
%This option is only available with the active-set algorithm. The default
% interior point algorithm will ignore any non-empty starting point.
%
% X=MADLIN(C,d,A,b,Aeq,Beq,LB,UB,X0,OPTIONS) minimizes with the default
...
%
% [X,RESNORM,RESIDUAL,EXITFLAG,OUTPUT] = MADLIN(C,d,A,b) returns a structure
...
% iterations (if used) in OUTPUT.cgiterations. The minimum absolute deviation
...
%
% [X,RESNORM,RESIDUAL,EXITFLAG,OUTPUT,LAMBDA]=MADLIN(C,d,A,b) returns the set of
% Lagrangian multipliers LAMBDA, at the solution of the LP: LAMBDA.ineqlin
% for the linear inequalities A, LAMBDA.eqlin for the linear equalities Aeq,
% LAMBDA.lower for LB, and LAMBDA.upper for UB.
%
% NOTE: the LargeScale (the default) version of MADLIN uses a primal-dual
...
% Any extra equality and inequality constraints can be appended to the LP.
