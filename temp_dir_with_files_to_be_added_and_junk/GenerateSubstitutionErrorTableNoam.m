...
% sequence
%
function p_error_mat = GenerateSubstitutionErrorTableNoam(k, ErrorStruct)

p_error_mat = zeros(k,4,4); % set matrix
p_one_nuc_error_mat = ErrorStruct.p_one_nuc_error_mat;
...
for i=1:4
    for j=1:4
        p_error_mat(:,i,j) = p .* p_one_nuc_error_mat(i,j);
        if(i==j)
