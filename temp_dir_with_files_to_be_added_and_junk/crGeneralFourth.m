function crGeneralFourth(basicPrefix,outFileDirName,numProcessors,bdf,nb,np,numIter,nr,addNoiseFlagInput,correctMeasurementForReadErrorsFlagInput,readLength,slave_queueName,master_queueName,upperLimitForRepeat,repeatWhenLowerThanThisValue,batchSize,brFlag,mkdirFlag,thresholdForCollectingBAC,groupSize,dataSetPrimers750Flag,CorrectReadsNew)

if length(numProcessors)>1
  if ~iscell(numProcessors)
    disp('convert numProcessors to cell. crGeneralFourth.m')
    return
  end
