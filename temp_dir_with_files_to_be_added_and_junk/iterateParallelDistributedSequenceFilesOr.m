function [X,sumRelevantReads]=iterateParallelDistributedSequenceFilesOr(uniqueReads,uniqueReads_length,curr_kp,basicSeqNameDir,basicSeqKey,parallelType,basicSaveName,userDir,runFileName,auxData)
%keyboard
    ...
    while length(find(numSuccessfullyCompleted))<length(partDist)-1
     ...
     %keyboard
      ...
      while txtFlag==0
       ...
       [j1,j2]=unix([...
              'grep -l "Successfully completed" ',runFileName,'_partDist_Z*.o | cut -f2 -d"Z" | cut -f1 -d"." >',runFileName,'_logOfSuccessful.txt;',...
              'grep -l "exit code" ',runFileName,'_partDist_Z*.o | cut -f2 -d"Z" | cut -f1 -d"." >',runFileName,'_logOfUnsuccessful1.txt;sleep 2;',...
              'grep -l "fatalexit" ',runFileName,'_partDist_Z*.o | cut -f2 -d"Z" | cut -f1 -d"." >',runFileName,'_logOfUnsuccessful2.txt;sleep 2;',...
              'grep -l "TERM_RUNLIMIT" ',runFileName,'_partDist_Z*.o | cut -f2 -d"Z" | cut -f1 -d"." >',runFileName,'_logOfUnsuccessful3.txt;',...
              'ls ',runFileName,'_logOfSuccessful.txt;',...
              'ls ',runFileName,'_logOfUnsuccessful1.txt;',...         
              'ls ',runFileName,'_logOfUnsuccessful2.txt;',...
              'ls ',runFileName,'_logOfUnsuccessful3.txt;',... 
                       ]);
       if isempty(findstr(j2,'ls: No match'))
         txtFlag = 1;
       end
      ...
      if processesAreStillRunningFlag==0
        ...
        if   length(resNames)==length(partDist)-1
          ...
          disp('running processes that were forgotten along the way. iterateParallelDistributedSequenceFilesOr.m')
