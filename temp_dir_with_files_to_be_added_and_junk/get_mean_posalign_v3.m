function [pos rev_pos sd rev_sd]=get_mean_posalign_v3(read, database1,database_size,bacseq_len)
...
if ~isempty(k)
    ...
    if ~isempty(k)
        ...
        pos=10000+mean(bacseq_len(J)+1+1-I-length(read));
        sd = 10000+std(bacseq_len(J)+1+1-I-length(read));
