function alignToDatabse(auxData)

% mcc 

% load the reads
load([auxData.dataDir,'/',auxData.dataName]);
...
runFileName = [auxData.userDir,'/CS/BAC/alignToDatabase/tmpRuns/alignTmp_',auxData.dataName,'/alignTmp_',auxData.dataName];
basicAllocationFileName = [auxData.userDir,'/CS/BAC/alignToDatabase/dataForSim/alignTmp_',auxData.dataName,'/alignTmp_',auxData.dataName];
currDir = [auxData.userDir,'/CS/BAC/alignToDatabase/tmpRuns/alignTmp_',auxData.dataName];
basicSaveName = [auxData.userDir,'/CS/BAC/alignToDatabase/alignTmp_',auxData.dataName];
...
if strcmp(parallelType,'cluster')
  ...
  for j=1:length(tmpInd)
    ...
    % run one part of alignment- new format
    runOneNodeAlignment(mFileName,saveFileName,reads_uni,tmpInd{j}(1),tmpInd{j}(end),auxData)
  end
  ...
  fprintf(fid,'bsub -W 1:30   -q  %s -W 1:59 -J "%s[1-%d]" -o "%s_partDist_Z%s.o" -e "%s_partDist_Z%s.e"  ~/runInput_donotdelete %s_partDist_%s;\n',auxData.queueName,tmpRunFileName,length(tmpInd),runFileName,'%I',runFileName,'%I',runFileName,'\$LSB_JOBINDEX');
  ...
  while length(find(numSuccessfullyCompleted))<length(tmpInd)
    ...
    if processesAreStillRunningFlag==0
      ...
      if   length(resNames)==length(tmpInd)
        ...
        JobList = [];
        JobList = runProblematic_alignment(listOfProblematic,runFileName,tmpRunFileName,auxData);
  ...
  for j=1:length(tmpInd)
    ...
    IDX(res.indexFirst:res.indexLast) = res.idx(res.indexFirst:res.indexLast);
    
  end
	
  unix(['rm -f ',currDir,'/core*; ','rm -f ',runFileName,'_*; ','rm -f ',basicSaveName,'_*; ']);
   
  save([basicSaveName,'_finalRes'],'IDX')	
