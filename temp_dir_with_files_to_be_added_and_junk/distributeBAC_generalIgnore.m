function distributeBAC_generalIgnore(auxDataFileName)
...
while length(curr_kp)>auxData.smallestSetOfCollected 
  ...
  if length(curr_kp)<20000 & auxData.repeatRandomGroups>1
    ...
    % save basic data for the runs
    fileToLoad = [basicSaveName,'_k_',num2str(k),'_basicData'];
    save(fileToLoad,'uniqueReads','uniqueReads_length','curr_kp','auxData','parallelType','userDir','k')

    % create the run files
    for l=1:auxData.repeatRandomGroups
      runFileName_repeatRandomGroups = [basicSaveName,'_k_',num2str(k),'_repeat_',num2str(l),'.exe'];
      runOneNodeRepeatRandomGroups(fileToLoad,l,runFileName_repeatRandomGroups)
    end
    unix(['chmod 0700 ',[basicSaveName,'_k_',num2str(k),'_repeat_*.exe']);
    
    % submit it
    
    
    % do the runs per number of CPU available
    
    % create the run file for this one
    
    

    % check that all  have finished
    resNames_repeatRandomGroups = dir([basicSaveName,'_k_',num2str(k),'_repeat_*','_tmpRes.mat']);
    while length(resNames_repeatRandomGroups)==0 || length(resNames_repeatRandomGroups)~=length(partDist)-1
      disp('checking whether it finished - repeatRandomGroups')
      pause(10)
      resNames_repeatRandomGroups = dir([basicSaveName,'_k_',num2str(k),'_repeat_*','_tmpRes.mat']);
    end
    ...
    % load the results
    for l=1:auxData.repeatRandomGroups
      cX = load([basicSaveName,'_k_',num2str(k),'_repeat_',num2str(l),'_tmpRes.mat'],'x');
