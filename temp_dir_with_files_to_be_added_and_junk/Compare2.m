...
function [freqset,igmd]=Compare2(mat,set1,freq1,set2,freq2,dist,Sequence1)
...
for a=1:length(set1)
    ...
    if (totsimilar>0)
        ...
        nosimilar=nosimilar+1;
%        disp(['No similar to ' num2str(a)]);
    end
end
disp(['No similar - ' num2str(nosimilar)]);

disp('Calculating max hamming distance');
maxdist=0;
 for cseq=(unique(used))'
     if (cseq>0)
         useq=find(used==cseq);
         for cc=1:length(useq)
             for dd=cc+1:length(useq)
                 [cscore,algn]=swalign(Sequence1{useq(cc)},Sequence1{useq(dd)},'Alphabet','NT');
                 cdist=sum(algn(1,:)~=algn(3,:));
