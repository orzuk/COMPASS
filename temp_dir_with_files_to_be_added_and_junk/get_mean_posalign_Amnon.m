function [pos rev_pos sd rev_sd]=get_mean_posalign_Amnon(read, database1,database_size,bacseq_len)
...
if ~isempty(k)
    ...
    pos=mean(I);
    sd = std(I);
    rev_pos = mean(bacseq_len(J)+1+1-I-length(read));
    rev_sd = std(bacseq_len(J)+1+1-I-length(read));
    ...
    if ~isempty(k)
        ...
        rev_pos=10000+mean(bacseq_len(J)+1+1-I-length(read));
        rev_sd = 10000+std(bacseq_len(J)+1+1-I-length(read));
