function [resCell]=loadResOr2(basicPrefix,outFileDirName,bdf,nb,np,numIter,nr)
        ...
        for nrr=1:length(nr)
          for ii=1:numIter
            ...
            try
              %keyboard
              %basicName
              rr = load([userDir,'/CS/BAC/',outFileDirName,'/',basicName]);
              resCell{bac_dist_flag+1,nbb,p,nrr,ii} = rr.resCell;
              %[bac_dist_flag+1,nbb,p,nrr,ii]
              %Nbac_in_mixture
              %pause
            catch
              basicName
