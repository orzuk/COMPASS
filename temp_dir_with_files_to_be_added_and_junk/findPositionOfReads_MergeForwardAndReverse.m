function findPositionOfReads_MergeForwardAndReverse(sampleName,auxData)
...
load([userDir,'/CS/BAC/12Samples/Solexa/data/',auxData.dirNameForData,'/data_',auxData.dirNameForData,'_',sampleName]);


n = size(uni_read_seq,1);
clear uni_read_seq
...
runFileName = [userDir,'/CS/BAC/findAlignment_MergeForwardAndReverse/tmpRuns/alignTmp_',num2str(readLength),'_',sampleName,'/alignTmp_',sampleName];
basicAllocationFileName = [userDir,'/CS/BAC/findAlignment_MergeForwardAndReverse/dataForSim/alignTmp_',num2str(readLength),'_',sampleName,'/alignTmp_',sampleName];
currDir = [userDir,'/CS/BAC/findAlignment_MergeForwardAndReverse/tmpRuns/alignTmp_',num2str(readLength),'_',sampleName];
basicSaveName = [userDir,'/CS/BAC/findAlignment_MergeForwardAndReverse/alignTmp_MergeForwardAndReverse_',num2str(readLength),'_',sampleName];
...
if strcmp(parallelType,'cluster')
  ...
  for j=1:length(tmpInd)
    ...
    % run one part - MergeFR - for this only
    runOneNodeBasedFindPositionOfReads_MergeFR(auxData.dirNameForData,sampleName,readLength,mFileName,saveFileName,tmpInd{j}(1),tmpInd{j}(end))
  end
  ...
  for j=1:length(tmpInd)
    ...
    REV_POS(res.indexFirst:res.indexLast) = res.rev_pos(res.indexFirst:res.indexLast);
    SD(res.indexFirst:res.indexLast) = res.sd(res.indexFirst:res.indexLast);
    REV_SD(res.indexFirst:res.indexLast) = res.rev_sd(res.indexFirst:res.indexLast);
  end
	
  unix(['rm -f ',currDir,'/core*; ','rm -f ',runFileName,'_*; ','rm -f ',basicSaveName,'_*; ']);
   
  save([basicSaveName,'_finalRes'],'POS','REV_POS','SD','REV_SD')	
