function [simulated_reads]=createReadsForSpecificMixture_package(auxData)
...
freq = auxData.correctWeight(ind_bac_in_mix);
numReadsBacteria = round(freq/sum(freq)*auxData.Nreads);
Nbac_in_mixture = length(ind_bac_in_mix); % number of bacteria in the mixture

% load sequences of bacteria in the mixture  %%%% NEED A FUNCTION FOR LOADING 
...
% list the kmer
currSeq = extract_sub_kmers(Sequence_mix{1}, len_mix(1), auxData.readLength, 0,0,1,1); % simulate one read, a stupid way of calculating the size of
                                                                                       % the vector in its packed form.
