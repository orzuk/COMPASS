...
% run_lin_prog - should be 0 if only calculates the dependent species, 1 if trying to solve and get bounds 
...
if(issparse(A)) % attempt to solve for sparse matrices (doesn't work yet. Function works only for full matrices (if sparse we can convert it to full but matrix may be way too large)
    	null_struct =  spqr_null(A);  %  SpLeft      = spspaces(A,1,10^-10); % get null-space for sparse A. From SuiteSparse package 
        null_basis = spqr_null_mult(null_struct,speye(size(null_struct.X,2)),1); % get basis in explicit (sparse) form         %    	null_basis = null_struct.X;        
else
    	null_basis = null(A); % get the null space
end
if ~isempty(null_basis)
    null_dim = size(null_basis,2)
    unique_inds = find( max(abs(null_basis),[],2) < epsilon ); % find species with zero entries in all elements of the null-space of A
else
    unique_inds = [1:n]';
end
...
if(run_lin_prog)
    ...
    end % loop on non-unique indices 
